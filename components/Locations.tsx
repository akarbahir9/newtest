    ...
    <div className="view-section active flex-1 p-4 md:p-8 overflow-y-auto relative">
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-end mb-6">
            <h1 className="text-2xl font-semibold text-zinc-100 tracking-tight">Locations</h1>
            <button 
          ...
          {currentProject.locations.map(loc => (
            <div key={loc.id} className="flex gap-4 p-4 bg-zinc-900 border border-zinc-800 rounded-lg">
                ...
                <div className="flex gap-2 mt-3">
                    <span className="text-xxs bg-zinc-800 px-2 py-1 rounded text-zinc-400">{loc.type === 'INT' ? 'Interior' : loc.type === 'EXT' ? 'Exterior' : 'Mixed'}</span>
                    <span className="text-xxs bg-zinc-800 px-2 py-1 rounded text-zinc-400">Set</span>
                </div>
                </div>
            </div>
          ))}
      ...
      {showModal && (
          <div className="absolute inset-0 bg-zinc-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl w-full max-w-md shadow-2xl">
                  <h3 className="text-lg font-medium text-zinc-100 mb-4">Add Location</h3>
                  <form onSubmit={handleSubmit} className="space-y-4">
                      <div>
                          <label className="block text-xs text-zinc-500 mb-1">Location Slug (e.g. HOUSE)</label>
