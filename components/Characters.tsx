...
const Characters: React.FC = () => {
  const { currentProject, addCharacter, updateCharacter } = useProject();
          ...
          {currentProject.characters.map(char => (
            <div key={char.id} onClick={() => setEditingChar(char)} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden hover:border-zinc-600 transition group cursor-pointer relative">
                    ...
                    {char.relationships && char.relationships.length > 0 ? (
                         <div className="flex flex-wrap gap-1">
                             {char.relationships.slice(0, 2).map((rel, i) => {
                                 ...
                                 return (
                                     <span key={i} className="text-[10px] bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded flex items-center gap-1">
                ...
                <div className="mt-4 pt-4 border-t border-zinc-800 flex gap-2 flex-wrap">
                    {char.traits.map((t, i) => (
                         <span key={i} className="px-2 py-1 bg-zinc-800 rounded text-[10px] text-zinc-400">{t}</span>
                    ))}
                </div>
                </div>
            </div>
